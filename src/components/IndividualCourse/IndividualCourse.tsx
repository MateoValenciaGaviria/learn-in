...
export const IndividualCourse: React.FC<IndividualCourseType> = ({ name, teacherName, progress, calification, important, updates, mainColor, teacherView }) => {
    ...
    return (
        <div className={`individual-course ${important === true ? ' individual-course--important' : ''}`} style={{ '--mainColor': mainColor } as React.CSSProperties} >
            <Link to={`/course-detail/${name}/&color=${mainColor}`} className={`individual-course__title-link ${updates > 0 ? ' individual-course__title-link--updates' : ''}`} data-content={`${updates ? updates : ''}`}><h2 className="individual-course__title">{name}</h2></Link>
            <Link to="/chat" className="individual-course__teacher-name">
                {!teacherView && <svg className="icon" width="19" height="19" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {teacherView === true && <svg className="icon" width="30" height="16" viewBox="0 0 31 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="15.5002" cy="4.88969" r="4.16667" stroke="white" />
                    <path d="M23 16.5564C23 12.4143 19.6421 9.0564 15.5 9.0564C11.3579 9.0564 8 12.4143 8 16.5564" stroke="white" />
                    <circle r="2.44194" transform="matrix(-1 0 0 1 5.39546 8.44194)" stroke="white" />
                    <path d="M1.00003 15.2794C1.00003 12.8518 2.96796 10.8839 5.39552 10.8839C6.5 10.8839 7.5 11.5 7.5 11.5" stroke="white" />
                    <circle cx="25.4419" cy="8.44194" r="2.44194" stroke="white" />
                    <path d="M29.8374 15.2794C29.8374 12.8518 27.8694 10.8839 25.4419 10.8839C24.3374 10.8839 23.3374 11.5 23.3374 11.5" stroke="white" />
                </svg>
                }
                {teacherName}
            </Link>
            ...
            {!teacherView && <p>Nota provisional: {calification} <span className="tooltip" data-content="Esta es tu nota contabilizando los valores que llevas hasta la fecha.">
